...
impl QuantizationContext {
  pub fn update(
    ...
    // a zero vs coding a one any any given coefficient position, or, in
    // the case of end-of-block bias, the cost of coding the block with
    // the eob was chosen (rounding to one) vs rounding to zero and continuing
    ...
    // biased derived via the method outlined in Jean-Marc Valin's
    ...
    //
    // In theory, the rounding biases for inter should be somwhat smaller
    ...
    // The RD-optimal rounding biases for the actual coefficients seems
    // to be quite closed, for both inter and intra, post-deadzoning.
    //
    ...
    self.ac_offset_tiny =
      self.ac_quant as i32 * (if is_intra { 96 } else { 61 }) / 256;
  }
    ...
    T: Coefficient,
  {
    ...
    // Here we uses different rounding biases depending on whether we've
    ...
    // coefficient is in the block, whereas the in the chunk of larger
    ...
    // Zero everything from the beginning of the deadzone to the end of
    // the block.
